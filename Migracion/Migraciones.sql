  ...
  BEGIN   
      ...
      BEGIN TRY      
        ...
        FROM dbo.Maestra m 
          ...
          JOIN dbo.Caja caja ON (m.CAJA_TIPO = c.tipo_caja_descripcion AND m.CAJA_NUMERO = c.caja_numero)
          JOIN dbo.Empleado emp ON (m.EMPLEADO_MAIL = emp.empleado_email) --como es unico podemos comparar con eso
          JOIN dbo.Sucursal suc ON ( (SELECT SUBSTRING_INDEX(m2.SUCURSAL_NOMBRE, ':', -1) FROM dbo.Maestra m2) = suc.sucursal_numero)
          -- Primero nos fijamos que la direccion coincida
          JOIN dbo.Provincia p ON (
            m.SUCURSAL_PROVINCIA = p.provincia_nombre 
            AND m.SUPER_PROVINCIA = p.provincia_nombre
        ...
        WHERE tc.id_tipo_comprobante IS NOT NULL
          ...
          AND m.TICKET_TOTAL_ENVIO IT NOT NULL
  ...
  END;
  ------------------------------------------------------------------------------------------------------------------------------------------------
  CREATE PROCEDURE dbo.migrar_promocion_x_item_ticket
  AS
  BEGIN   
    BEGIN TRANSACTION;
    BEGIN TRY
  ...
  BEGIN   
      ...
      FROM dbo.Maestra m
        JOIN dbo.Tipo_medio_de_pago mp ON (m.PAGO_TIPO_MEDIO_PAGO = mp.tipo_medio_pago_nombre)
        JOIN dbo.Cuota cuota ON (m.PAGO_TARJETA_CUOTAS = cuota.cuota) --revisar 
        JOIN dbo.Cliente cl ON (m.CLIENTE_MAIL = cl.cliente_mail) --se puede porque es unico el mail para cada cliente
        JOIN dbo.Descuento dsc ON(m.DESCUENTO_CODIGO = dsc.descuento_codigo)
      WHERE 
